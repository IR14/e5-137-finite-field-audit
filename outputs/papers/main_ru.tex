...
\begin{equation}
    z_{\rm Planck}
    =
    \frac{3.0570989176262793-2.99}{0.17}
    =
    0.39469951544870036
\end{equation}
стандартных отклонений.

В этом сравнении также фиксируется нормировка информационной скорости,
используемая в вычислительном аудите.  Планковская плотность битрейта вводится
как учетная шкала:
\begin{equation}
    \rho_{\dot I,{\rm Pl}}
    \simeq
    ...
    ~{\rm bits~s^{-1}~cm^{-3}}.
\end{equation}
Она не интерпретируется как измеренная пропускная способность модели.  В
тексте она используется только для записи сокращения числа битов и стоимости
конечнополевого обновления в общей размерности.
...
Энергетическая учетная оценка в релизе использует нижнюю границу Ландауэра для
стирания $B$ битов при температуре $T$:
\begin{equation}
    E_{\rm L}(B,T)=B k_{\rm B}T\ln2.
\end{equation}
Для контрольной точки с $P=2113$ символами FP32-представление требует
\begin{equation}
    B_{\rm FP32}=32P=67616~{\rm bits}.
\end{equation}
Восстанавливаемая конечнополевая контрольная точка HYP-007 хранит $R=3458$
байтовых символов:
\begin{equation}
    B_{\rm RS(26,16)}=8R=27664~{\rm bits},
    \qquad
    \frac{E_{\rm L}(B_{\rm RS},T)}{E_{\rm L}(B_{\rm FP32},T)}
    =
    0.40913393279697113.
\end{equation}
По выпущенному набору запусков HYP-007 среднее отношение равно
\begin{equation}
    \left\langle
    \frac{E_{\rm L}(B_{\rm RS},T)}{E_{\rm L}(B_{\rm FP32},T)}
    \right\rangle
    ...
    0.4111824864372086,
\end{equation}
что соответствует сокращению битовой учетной оценки на
$0.5888175135627913$.  Это расчет по числу хранимых битов, а не прямое
измерение потребляемой мощности.

\begin{table}[h]
\centering
\caption{Аудит памяти и времени для периферийного инференса.}
\small
\setlength{\tabcolsep}{3pt}
...
GF(137) C++ объединенный \texttt{uint8\_t} & 1.62793 & 40.1530 ms & внешний вызов \\
...
арифметике.  Примитив \texttt{mac137} может использовать редукцию Барретта с
\begin{equation}
    \mu=\left\lfloor\frac{2^{32}}{137}\right\rfloor=31350126,
\end{equation}
после чего выполняются корректирующие вычитания.  Примитив \texttt{inv137} не
находится в горячем цикле инференса и может быть реализован как
\begin{equation}
    {\tt inv137}(a)=a^{135}\bmod137
\end{equation}
или через таблицу.  Стоимость одного такого примитива ограничена как $O(1)$.
Двухслойное плотное ядро аудита имеет сложность $O(RKH+RH)$ по размерам
матриц; общий квадратный dense GEMM остается задачей $O(N^3)$.
