...
The CMB comparison also fixes the information-rate normalization used in the
computational audit.  The Planck-scale bit-rate density is treated as a
bookkeeping scale,
\begin{equation}
    \rho_{\dot I,{\rm Pl}}
    \simeq
    ...
    ~{\rm bits~s^{-1}~cm^{-3}},
\end{equation}
not as a measured throughput of the model.  It is used only to express
storage-bit reductions and finite-field update costs on a common scale.
...
The energy-cost proxy used in the release archive is the Landauer lower bound
for erasing $B$ bits at temperature $T$,
\begin{equation}
    E_{\rm L}(B,T)=B k_{\rm B}T\ln2.
\end{equation}
For a checkpoint with $P=2113$ field symbols, the FP32 representation uses
\begin{equation}
    B_{\rm FP32}=32P=67616~{\rm bits}.
\end{equation}
The repaired finite-field checkpoint used in HYP-007 stores
$R=3458$ byte-valued symbols, hence
\begin{equation}
    ...
    0.40913393279697113.
\end{equation}
Across the released HYP-007 sweep, the mean repaired-checkpoint ratio is
\begin{equation}
    \left\langle
    \frac{E_{\rm L}(B_{\rm RS},T)}{E_{\rm L}(B_{\rm FP32},T)}
    \right\rangle
    ...
    0.4111824864372086,
\end{equation}
corresponding to a storage-bit energy proxy reduction of
$0.5888175135627913$.  This is an accounting result, not a direct power
measurement.
...
\end{align}
The relevant engineering property is bounded integer arithmetic.  The
\texttt{mac137} primitive can use Barrett reduction with
\begin{equation}
    \mu=\left\lfloor\frac{2^{32}}{137}\right\rfloor=31350126,
\end{equation}
followed by one or more subtractive corrections.  The \texttt{inv137}
primitive is outside the hot inference loop and may be implemented as
\begin{equation}
    {\tt inv137}(a)=a^{135}\bmod137
\end{equation}
by Fermat inversion or by a lookup table.  A single primitive has bounded
$O(1)$ cost; the two-layer dense audit kernel remains
$O(RKH+RH)$ in the matrix dimensions, while a generic square dense GEMM is
$O(N^3)$.
