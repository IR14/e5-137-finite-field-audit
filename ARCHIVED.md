...
Release 1.0 / Production Ready appendix:

- status: RELEASE 1.0 / PRODUCTION READY / ARCHIVED READ-ONLY after final
  EFT horizon time-dilation audit
- final field-dynamics script:
  `scripts/lagrangian_e5137_interaction_optimizer.py`
- final report: `outputs/reports/e5_137_interaction_optimizer.md`
- final scan table: `outputs/tables/e5_137_interaction_optimizer.csv`
- final trajectory table:
  `outputs/tables/e5_137_interaction_optimizer_best_trajectories.csv`
- paper update: `papers/e5_137_lepton_generation/main.tex`, Section 4.4,
  "Lagrangian Field Dynamics and Horizon-Gated Limitations"
- result: active EFT null result; no compensating tails, coupling multipliers,
  post-hoc phase shifts, or DESI-target fitting are retained
- accepted crossing row: `w0 = -0.920331`, `w(z=1) = -1.00259`, target
  `w(z=1) = -1.18203`, residual `0.179442`
- horizon gate strength: `q/117 = 7.57317e-05`, `max H_gate ~= 1.00028`,
  best-row `Delta w(z=1) = 2.29427e-07`
- failed forced-depth row: `w(z=1) = -1.13915`, but
  `Omega_dark(z>10)_max = 0.458652`, `early_sanity = False`, and no accepted
  sampled phantom crossing
- interpretation: the proposed horizon time-dilation factor is rejected inside
  this EFT ansatz; the lambda-like dark-energy component is not treated as an
  observer-time illusion of an event-horizon phase
- anti-p-hacking guardrail: the branch is archived precisely because the
  tested mechanism failed its own preregistered numerical gates
- final validation: `uv run --extra test pytest` -> 5 passed on 2026-05-30
- filesystem lock: repository tree marked read-only with the release state
  preserved; this is an operating-system permission lock, not a scientific
  argument and not a substitute for the archive reports above

CMB N_eff closure appendix:

- status: FINAL TRILOGY LOCK after CMB radiation-trace and matter-power cutoff
  audit
- source function: `src/cosmo_gradient/theory.py::cmb_neff_validation`
- report: `outputs/reports/cmb_neff_validation.md`
- paper update: `papers/e5_137_lepton_generation/main.tex`, Section 4.5,
  "CMB Anisotropy, N_eff Invariants, and Matter Power Spectrum Cutoff"
- invariant:
  `Delta N_eff = (D / 137) * delta_phi * (1 - q)`
- result: `Delta N_eff = 0.011098917626279356`,
  `N_eff = 3.0570989176262793`
- comparison window: `N_eff = 2.99 +/- 0.17`, z-score `0.39469951544870036`
- matter cutoff audit: thermal-equivalent
  `lambda_cut = 6.097325934119188e-7 kpc`,
  `k_cut = 1.0304821121699224e10 Mpc^-1`
- LSS/BAO guardrail: cutoff is far below `100 kpc`, so under the archived
  fully sterile cold/nonthermal production assumption the benchmark is
  CDM-like on DESI DR1 null-result scales
- limitation: the mass and `N_eff` invariant still do not derive relic
  abundance or a production momentum distribution without a production model
- final validation: `uv run pytest` -> 5 passed on 2026-05-30

Phase 10 GF(137) hash-archive appendix:

- status: FOREVER MAXIMUM HARD LOCK after semantic hash-archive benchmark
- C++ kernel: `src/modular_compression/e5137_extractor.cpp`
- Python binding: `src/cosmo_gradient/modular_compression.py`
- shootout script: `scripts/e5137_compressor_shootout.py`
- report: `outputs/reports/e5137_extractor_manifest.md`
- table: `outputs/tables/e5137_extractor_shootout.csv`
- representation: 26 axes * 5 subparticles = 130 bytes per super-vector
- semantic cap: Scenario C is archived as exactly `117 * 130 = 15210` bytes
- benchmark rows:
  A `954 -> 130 bytes`, ratio `7.33846`;
  B `9571 -> 130 bytes`, ratio `73.6231`;
  C `765648 -> 15210 bytes`, ratio `50.3385`
- repair audit: deleting 10 of 26 axes leaves 16 votes; majority repair
  exactly restores the archived GF(137) super-vectors for A/B/C
- guardrail: this is a lossy semantic/hash archive and does not claim
  universal lossless text decompression without an external source model
- final validation: `uv run pytest` -> 5 passed on 2026-05-30

Nuclear Chemistry / Iron-26 appendix:

- status: TERMINAL SHUTDOWN after nuclear mass-defect audit
- source function: `src/cosmo_gradient/theory.py::nuclear_chemistry_validation`
- report: `outputs/reports/nuclear_chemistry_validation.md`
- paper update: `papers/e5_137_lepton_generation/main.tex`,
  "Quantum-Information Chemistry and the Iron-26 Resonance"
- formula tested:
  `Delta M = Z^2 * E_step(Z) * [1 + 42/(137D) * (1 - 13/117)]`
- running step:
  `E_step(1) = 5e eV`,
  `E_step(Z>1) = 5e * (137*42) * (1 - D/(137N)) = 75236.60444808945 eV`
- structural resonance: `Z_Fe = 26 = D_STRING`
- Fe-56 reference defect: `492.2595698447403 MeV`
- Fe-56 predicted defect: `51.393007848156245 MeV`
- Fe-56 mass residual: `8463.590833506674 ppm`
- release gate: failed; He-4, C-12, and Fe-56 do not pass the `<10 ppm`
  nuclear-mass criterion
- guardrail: the iron-26 equality is retained only as a mnemonic structural
  resonance, not as a derivation of nuclear binding or the iron stability peak
